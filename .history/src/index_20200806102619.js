...
document.addEventListener("DOMContentLoaded", () => {
  ...
  addBtn.addEventListener("click", () => {
    ...
    if (addToy) {
      ...
      document.getElementsByClassName('add-toy-form').addEventListener('submit', addNewToy(document.getElementsByClassName('input-text')[0], document.getElementsByClassName('input-text')[1]))
    } else {
...
let addNewToy = (toyName, toyImage) => {
  ...
  return fetch('http://localhost:3000/toys',configObj)
    .then(resp => resp.json())
    .then(json => {
      document.getElementById('toy-collection').append(json)
    });
};
